...
func (t *Templater) Render(w http.ResponseWriter, name string, data any, status int) {
    ...
    // Execute the requested template. Child templates are expected to invoke the "base"
    // template via {{template "base" .}}. This allows overriding the "title" and "content"
    // blocks defined in base.
